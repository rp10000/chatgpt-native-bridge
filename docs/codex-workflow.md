...
2. Prefer the desktop client path. If I want a GPT-5.5 Pro planning pass, run or ask me to run `npx --yes --package github:rp10000/chatgpt-native-bridge -- cgn start`.
...
6. After the client imports a Pro reply, ChatGPT writes back through MCP, or I import the reply with `cgn done`, read `reply.md` and `CODEX_READ_THIS.md`.
...
Prefer the desktop client for GPT-5.5 Pro planning. If the client and MCP are unavailable, create a handoff with --type plan,requirements,
...
```text
After implementation, prefer the desktop client or MCP for diff review. If both are unavailable, create
