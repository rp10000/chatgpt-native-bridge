...
如果没有全局安装 `cgn`，后续命令都可以写成 `npx github:rp10000/chatgpt-native-bridge <command>`。
...
如果 ChatGPT 不能直接访问本机 `127.0.0.1`，请使用官方 MCP / Apps SDK / Secure MCP Tunnel 连接方式。

ChatGPT 最终应通过 `submit_reply_to_codex` 把建议写回本地 inbox。
